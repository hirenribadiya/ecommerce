...
export const Kids = () => {
  ...
  // Fetch products from the backend for men category
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("http://localhost:5000/seller?category=kids");
        const data = await response.json();
        setProducts(data);
        setFilteredProducts(data); // Initially show all men's products
      } catch (error) {
